...
-- Verification query: SELECT to_regclass('public.idx_outbox_processed_retention') IS NOT NULL AND to_regclass('public.idx_outbox_dead_letter_retention') IS NOT NULL AND has_function_privilege('pharmacycrm_runtime','public.delete_processed_outbox_events_before(timestamptz,integer)','EXECUTE') AND has_function_privilege('pharmacycrm_runtime','public.delete_dead_letter_outbox_events_before(timestamptz,integer)','EXECUTE') AND (SELECT bool_and(prosecdef) FROM pg_proc WHERE oid IN ('public.delete_processed_outbox_events_before(timestamptz,integer)'::regprocedure,'public.delete_dead_letter_outbox_events_before(timestamptz,integer)'::regprocedure)) AND NOT has_table_privilege('pharmacycrm_runtime','outbox_events','DELETE');
