...
-- Verification query: SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='inventory_movements' AND column_name='lot_sequence' AND data_type='bigint' AND is_nullable='NO') AND EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='public.inventory_movements'::regclass AND conname='uq_inventory_movement_lot_sequence' AND contype='u' AND convalidated) AND EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='public.inventory_movements'::regclass AND conname='chk_inventory_movement_lot_sequence' AND convalidated) AND EXISTS (SELECT 1 FROM pg_trigger WHERE tgrelid='public.inventory_movements'::regclass AND tgname='trg_assign_inventory_movement_lot_sequence' AND tgenabled='O' AND NOT tgisinternal) AND to_regclass('public.idx_inventory_movements_lot_history') IS NULL AND NOT has_table_privilege('pharmacycrm_runtime','inventory_movements','UPDATE,DELETE,TRUNCATE');
