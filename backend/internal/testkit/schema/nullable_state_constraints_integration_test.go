...
func TestNullableLifecycleConstraintsIntegration(t *testing.T) {
	...
	tests := []struct {
		...
		{"session active", `INSERT INTO user_sessions(id,user_id,refresh_token_hash,token_family_id,expires_at) VALUES($4,$1,convert_to(gen_random_uuid()::text,'UTF8'),gen_random_uuid(),now()+interval '1 day')`, false},
		{"session revoked", `INSERT INTO user_sessions(id,user_id,refresh_token_hash,token_family_id,expires_at,revoked_at,revoke_reason) VALUES($4,$1,convert_to(gen_random_uuid()::text,'UTF8'),gen_random_uuid(),now()+interval '1 day',now(),'rotation')`, false},
		{"session null reason", `INSERT INTO user_sessions(id,user_id,refresh_token_hash,token_family_id,expires_at,revoked_at,revoke_reason) VALUES($4,$1,convert_to(gen_random_uuid()::text,'UTF8'),gen_random_uuid(),now()+interval '1 day',now(),NULL)`, true},
		{"session blank reason", `INSERT INTO user_sessions(id,user_id,refresh_token_hash,token_family_id,expires_at,revoked_at,revoke_reason) VALUES($4,$1,convert_to(gen_random_uuid()::text,'UTF8'),gen_random_uuid(),now()+interval '1 day',now(),'   ')`, true},
		{"session partial state", `INSERT INTO user_sessions(id,user_id,refresh_token_hash,token_family_id,expires_at,revoked_at,revoke_reason) VALUES($4,$1,convert_to(gen_random_uuid()::text,'UTF8'),gen_random_uuid(),now()+interval '1 day',NULL,'rotation')`, true},
