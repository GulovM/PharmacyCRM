...
func TestCanonicalLocksSerializeAndReturnSortedProductsIntegration(t *testing.T) {
	...
	otherSaleOperationID, otherSaleID, otherSaleItemID, otherAllocationID := uuid.New(), uuid.New(), uuid.New(), uuid.New()
	foreignPharmacyID, foreignPharmacyProductID := uuid.New(), uuid.New()
	foreignOperationID, foreignReceiptID, foreignReceiptItemID, foreignLotID := uuid.New(), uuid.New(), uuid.New(), uuid.New()
	receiptItemOne, receiptItemTwo, saleItemOne, saleItemTwo := uuid.New(), uuid.New(), uuid.New(), uuid.New()
	if bytes.Compare(saleItemOne[:], saleItemTwo[:]) > 0 {
		saleItemOne, saleItemTwo = saleItemTwo, saleItemOne
	}
	...
	t.Cleanup(func() {
		_, _ = pool.Exec(context.Background(), "DELETE FROM stock_lots WHERE id = $1", foreignLotID)
		_, _ = pool.Exec(context.Background(), "DELETE FROM receipt_items WHERE id = $1", foreignReceiptItemID)
		_, _ = pool.Exec(context.Background(), "DELETE FROM receipts WHERE id = $1", foreignReceiptID)
		_, _ = pool.Exec(context.Background(), "DELETE FROM inventory_operations WHERE id = $1", foreignOperationID)
		_, _ = pool.Exec(context.Background(), "DELETE FROM pharmacy_products WHERE id = $1", foreignPharmacyProductID)
		_, _ = pool.Exec(context.Background(), "DELETE FROM pharmacies WHERE id = $1", foreignPharmacyID)
		_, _ = pool.Exec(context.Background(), "DELETE FROM sale_item_allocations WHERE id IN ($1,$2,$3)", allocationFirst, allocationSecond, otherAllocationID)
		_, _ = pool.Exec(context.Background(), "DELETE FROM sale_items WHERE id IN ($1,$2,$3)", saleItemOne, saleItemTwo, otherSaleItemID)
		_, _ = pool.Exec(context.Background(), "DELETE FROM sales WHERE id IN ($1,$2)", saleID, otherSaleID)
		...
		_, _ = pool.Exec(context.Background(), "DELETE FROM inventory_operations WHERE id IN ($1,$2,$3)", receiptOperationID, saleOperationID, otherSaleOperationID)
	...
	if _, err := pool.Exec(ctx, "INSERT INTO pharmacies (id,name,address,latitude,longitude) VALUES ($1,'Foreign Lock Test','Test',0,0)", foreignPharmacyID); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := pool.Exec(ctx, `INSERT INTO pharmacy_products(id,pharmacy_id,product_presentation_id,default_package_price_dirams) VALUES($1,$2,$3,100)`, foreignPharmacyProductID, foreignPharmacyID, presentationOne); err != nil {
		t.Fatal(err)
	}
	if _, err := pool.Exec(ctx, `INSERT INTO inventory_operations (id,pharmacy_id,operation_type,initiated_by_user_id,occurred_at) VALUES
		($1,$4,'RECEIPT',$5,now()),($2,$4,'SALE',$5,now()),($3,$4,'SALE',$5,now())`, receiptOperationID, saleOperationID, otherSaleOperationID, pharmacyID, userID); err != nil {
		t.Fatal(err)
	}
	if _, err := pool.Exec(ctx, `INSERT INTO inventory_operations(id,pharmacy_id,operation_type,initiated_by_user_id,occurred_at) VALUES($1,$2,'RECEIPT',$3,now())`, foreignOperationID, foreignPharmacyID, userID); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := pool.Exec(ctx, `INSERT INTO receipts(id,pharmacy_id,operation_id,receipt_number,received_at,posted_by_user_id,posted_at) VALUES($1::uuid,$2,$3,($1::uuid)::text,now(),$4,now())`, foreignReceiptID, foreignPharmacyID, foreignOperationID, userID); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := pool.Exec(ctx, `INSERT INTO receipt_items(id,receipt_id,pharmacy_product_id,batch_number,expiration_date,quantity_packages,base_units_per_package_snapshot,quantity_base_units,purchase_price_package_dirams,retail_price_package_dirams) VALUES($1,$2,$3,'foreign','2032-01-01',1,1,1,50,100)`, foreignReceiptItemID, foreignReceiptID, foreignPharmacyProductID); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := pool.Exec(ctx, `INSERT INTO stock_lots(id,pharmacy_product_id,receipt_item_id,origin_type,batch_number,expiration_date,quantity_base_units,base_units_per_package_snapshot,purchase_price_package_dirams,package_retail_price_dirams,received_at) VALUES($1,$2,$3,'RECEIPT','foreign','2032-01-01',1,1,50,100,now())`, foreignLotID, foreignPharmacyProductID, foreignReceiptItemID); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := pool.Exec(ctx, `INSERT INTO sales (
		id,pharmacy_id,operation_id,sale_number,payment_method,subtotal_amount_dirams,
		discount_amount_dirams,total_amount_dirams,sold_by_user_id,sold_at
		) VALUES ($1::uuid,$2,$3,($1::uuid)::text,'CASH',100,0,100,$4,now())`, otherSaleID, pharmacyID, otherSaleOperationID, userID); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := pool.Exec(ctx, `INSERT INTO sale_items (
		...
		display_quantity,base_units_per_package_snapshot,quantity_base_units,unit_price_dirams,
		line_subtotal_dirams,line_total_dirams
		) VALUES ($1,$2,$3,'Other','box','PACKAGE',1,1,1,100,100,100)`, otherSaleItemID, otherSaleID, first); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := pool.Exec(ctx, `INSERT INTO sale_item_allocations(id,sale_item_id,stock_lot_id,quantity_base_units) VALUES($1,$2,$3,1)`, otherAllocationID, otherSaleItemID, lotLater); err != nil {
		t.Fatal(err)
	}
	...
	returnPlan := locking.SaleReturnPlan{
		PharmacyID: pharmacyID, SaleID: saleID,
		SourceSaleItemIDs:   []uuid.UUID{saleItemTwo, saleItemOne},
	...
	returnLocks, err := returnRepository.LockSaleReturn(ctx, returnPlan)
	if err != nil {
	...
	if returnLocks.Sale.ID != saleID ||
		returnLocks.SourceSaleItems[0].ID != saleItemOne ||
	...
	expectMissing := func(name string, plan locking.SaleReturnPlan) {
		t.Helper()
		t.Run(name, func(t *testing.T) {
			...
			repository, _ := NewCanonicalLockRepository(tx)
			if _, err := repository.LockSaleReturn(ctx, plan); !errors.Is(err, locking.ErrLockTargetMissing) {
				t.Fatalf("expected missing target, got %v", err)
			}
		})
	}
	foreignItemPlan := returnPlan
	foreignItemPlan.SourceSaleItemIDs = []uuid.UUID{saleItemOne, otherSaleItemID}
	expectMissing("sale item from another sale", foreignItemPlan)
	foreignAllocationPlan := returnPlan
	foreignAllocationPlan.SourceAllocationIDs = []uuid.UUID{allocationFirst, otherAllocationID}
	expectMissing("allocation from another sale", foreignAllocationPlan)
	incompletePlan := returnPlan
	incompletePlan.SourceAllocationIDs = []uuid.UUID{allocationFirst}
	incompletePlan.StockLotIDs = []uuid.UUID{lotLater}
	expectMissing("sale item without selected allocation", incompletePlan)
	foreignLotPlan := returnPlan
	foreignLotPlan.StockLotIDs = []uuid.UUID{lotEarlier, foreignLotID}
	expectMissing("lot from another pharmacy", foreignLotPlan)
	missingPlan := returnPlan
	missingPlan.SourceSaleItemIDs = []uuid.UUID{saleItemOne, uuid.New()}
	expectMissing("missing id", missingPlan)

	// The same return working set serializes in the canonical order regardless
	// of the caller's input order.
	txReturnOne, err := pool.Begin(ctx)
	if err != nil {
		t.Fatal(err)
	}
	firstReturnRepository, _ := NewCanonicalLockRepository(txReturnOne)
	if _, err := firstReturnRepository.LockSaleReturn(ctx, returnPlan); err != nil {
		t.Fatal(err)
	}
	returnResult := make(chan error, 1)
	go func() {
		txReturnTwo, err := pool.Begin(ctx)
		if err != nil {
			returnResult <- err
			return
		}
		secondReturnRepository, _ := NewCanonicalLockRepository(txReturnTwo)
		orderedPlan := returnPlan
		orderedPlan.SourceSaleItemIDs = []uuid.UUID{saleItemOne, saleItemTwo}
		orderedPlan.SourceAllocationIDs = []uuid.UUID{allocationFirst, allocationSecond}
		orderedPlan.PharmacyProductIDs = []uuid.UUID{first, second}
		orderedPlan.StockLotIDs = []uuid.UUID{lotEarlier, lotLater}
		_, err = secondReturnRepository.LockSaleReturn(ctx, orderedPlan)
		if err == nil {
			err = txReturnTwo.Commit(ctx)
		} else {
			_ = txReturnTwo.Rollback(ctx)
		}
		returnResult <- err
	}()
	select {
	case err := <-returnResult:
		t.Fatalf("concurrent return lock did not serialize: %v", err)
	case <-time.After(100 * time.Millisecond):
	}
	if err := txReturnOne.Commit(ctx); err != nil {
		t.Fatal(err)
	}
	select {
	case err := <-returnResult:
		if err != nil {
			t.Fatalf("serialized return lock failed: %v", err)
		}
	case <-time.After(2 * time.Second):
		t.Fatal("serialized return lock timed out")
	}
}
