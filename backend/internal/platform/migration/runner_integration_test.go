...
func TestUpgradeFromE1Integration(t *testing.T) {
	...
	if len(loaded) != 14 || loaded[0].Version != 1 || loaded[0].Name != "schema_metadata" {
	...
	if result.SchemaVersion != 14 || len(result.Applied) != 13 || result.Applied[0] != 2 || result.Applied[len(result.Applied)-1] != 14 {
	...
	if replayed.SchemaVersion != 14 || len(replayed.Applied) != 0 {
