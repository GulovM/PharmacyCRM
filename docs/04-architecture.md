...
## Инкорпорированные архитектурные правила
...
Минимальная целевая декомпозиция MVP:

| Модуль | Ответственность |
|---|---|
| `identity` | пользователи, аутентификация, сессии, блокировка и роли |
| `pharmacy` | аптеки, публичные данные точки, назначения аптекарей |
| `catalog` | глобальный каталог, представления препаратов, штрихкоды, staging и модерация |
| `assortment` | ассортимент конкретной аптеки, локальные цены и правила отпуска |
| `inventory` | поступления, лоты, остатки, движения, списания и корректировки |
| `sales` | продажи, строки продажи, расчёт суммы, FEFO-распределение и проведение |
| `returns` | возвраты, связь с продажей, распределение возвращаемого количества и допустимость возврата в остаток |
| `search` | публичная проекция наличия и поиск аптек с доступным товаром |
| `alerts` | предупреждения по срокам годности и минимальным остаткам |
| `replenishment` | рекомендации по ручному пополнению |
| `audit` | неизменяемая фиксация критических действий и системных инициаторов |

Физическое объединение близких модулей на первом этапе допустимо, если их публичные интерфейсы и владение данными остаются явными. Например, `returns` может временно находиться рядом с `sales`, но не должен превращаться в неструктурированный общий пакет.
...
Критические команды поддерживают idempotency key в пределах scope, установленного SRS и API design.

Обработка выполняется внутри той же транзакции, что и бизнес-эффект:

1. определяется scope ключа;
2. вычисляется канонический hash смыслового payload;
3. создаётся или блокируется запись идемпотентности;
4. при совпадении ключа и hash возвращается исходный результат;
5. при совпадении ключа и различии hash возвращается конфликт;
6. новый бизнес-эффект и результат сохраняются атомарно.

Нельзя сначала провести операцию, а затем отдельно записать idempotency key.

Транспортные поля, не меняющие смысл команды, не должны случайно изменять канонический hash. Правила канонизации фиксируются в API design.
...
## 15. Внутренние события и post-commit действия

Domain events могут описывать уже совершившийся факт, например `SaleCompleted` или `StockLotExpired`.

В рамках MVP события могут обрабатываться внутри процесса. Однако действие, влияющее на внешнюю систему или eventual-consistent проекцию, не должно выполняться до успешного commit основной транзакции.

Допустимые стратегии:

- выполнить лёгкое post-commit действие с безопасным retry;
- записать transactional outbox в той же транзакции и обработать фоновым worker;
- перестроить производную проекцию из источника истины.

Transactional outbox обязателен, когда потеря события после commit создаёт недопустимое расхождение. Его фактическое внедрение определяется ADR для конкретной интеграции.
...
## 28. Открытые архитектурные вопросы

До production должны быть окончательно зафиксированы:

1. конкретная модель access/refresh sessions и отзыв токенов;
2. необходимость transactional outbox для поисковой проекции и будущих интеграций;
3. допустимая задержка публичного наличия;
4. модель запуска workers: отдельный процесс или режим общего бинарника;
5. стратегия восстановления поисковой проекции;
6. политика retry для serialization failure и deadlock;
7. границы хранения и архивирования аудита;
8. резервное копирование и проверка восстановления PostgreSQL;
9. целевые SLO и лимиты нагрузки;
10. нормативные условия возврата лекарств и проверки рецепта.

Открытый вопрос не отменяет уже установленные инварианты и не разрешает временно обходить их.
