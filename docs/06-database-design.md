...
Текущий E2 schema contract — version `21`; обязательный upgrade path начинается с immutable E1 version `1` и применяет forward migrations `000002..000021`. `user_sessions` хранит generation, idle/absolute expiry, authentication method и MFA level; ротация ссылается composite FK только на предыдущую generation того же user и token family.
