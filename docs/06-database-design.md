...
6. Поле `auth_version` не существует. Актуальность доступа определяется current user/session/role/assignment/pharmacy records.
...
3. `READY_FOR_REVIEW`, `QUEUED`, `RUNNING`, `SUCCEEDED` не являются persisted `ImportJob` states.
...
## 11. Открытые решения до production

1. Legal policy возвратов лекарств.
2. Partial refund allocation и rounding.
3. Password/session retention и recovery policy.
4. Idempotency retention и policy для stable 4xx result.
5. Outbox lease duration, batch size, attempts, retention и operator retry workflow.
6. Формат `working_hours`.
7. Full-text/trigram search strategy.
8. Audit partitioning только после измерения объёма.
9. Quarantine/return-lot suitability rules.
10. Elevated approval policy для adjustments/reversals.
11. Public projection TTL и stale-display policy.
12. Production PostgreSQL major-upgrade policy.
