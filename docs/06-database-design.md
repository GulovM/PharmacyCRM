...
12. Возвратная mutation остаётся выключенной в production до утверждения юридической policy и refund/rounding rules.
...
3. `legacy review-ready transport label`, `QUEUED`, `RUNNING`, `SUCCEEDED` не являются persisted `ImportJob` states.
...
13. Return command production-disabled до legal и refund/rounding approval.
