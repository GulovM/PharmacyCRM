...
| 15 | `15-implementation-specification.md` | подробное исполнимое ТЗ: этапы, функции, входы, ошибки, безопасность, транзакции, тесты и критерии приёмки | Draft |

## 4. Статус основного комплекта

Основной проектный комплект документов `00–15` создан. Новые верхнеуровневые документы добавляются только при появлении отдельной нормативной области, которую нельзя корректно включить в существующие документы.

`15-implementation-specification.md` не является копией остальных документов. Он связывает утверждённые требования в единый поэтапный implementation backlog и обязан синхронизироваться с источниками истины при изменении feature, contract, schema, security control, gate или acceptance criteria.
...
- сопровождения feature IDs, blockers и acceptance criteria из Implementation Specification;
...
1. затронутые SRS, Architecture, API, Domain Model, Project Structure, Security Design, Sequence Diagrams, Development Roadmap, Deployment, Testing Strategy, Observability, Implementation Specification и Database Design не противоречат друг другу;
...
7. HTTP feature обновляет `05-api-design.md`, OpenAPI и соответствующую feature card в `15-implementation-specification.md` в том же change set;
8. изменение схемы синхронизирует Database Design, migrations и применимые feature requirements;
9. изменение aggregate boundary или критического доменного инварианта синхронизирует `07-domain-model.md`, Implementation Specification и при необходимости оформляется ADR;
10. изменение backend module/package ownership синхронизирует `04-01-backend-architecture.md`, `08-project-structure.md` и Implementation Specification;
...
13. изменение authentication, authorization, sessions, secrets, audit или public/private data boundary синхронизирует `09-security-design.md`, Implementation Specification и при необходимости оформляется ADR;
14. изменение transaction boundary, lock order, idempotency protocol, transactional audit или post-commit flow синхронизирует `10-sequence-diagrams.md` и соответствующую feature card;
15. изменение MVP scope, зависимостей этапов, release gate, Definition of Ready/Done или приоритета P0/P1 синхронизирует `11-development-roadmap.md` и Implementation Specification;
...
17. изменение acceptance criteria, authorization matrix, persistence behavior, concurrency protocol, contract, migration, security, frontend workflow, performance baseline или recovery procedure синхронизирует `13-testing-strategy.md`, Implementation Specification и соответствующие automated suites;
...
19. feature ID, blocker ID и acceptance criteria остаются трассируемыми до tests/CI evidence;
20. архитектурно значимое решение оформлено ADR;
21. примеры кода и диаграммы не противоречат принятым ADR и текущему стеку.
...
Implementation Specification создан как рабочая точка входа в реализацию `E1–E12`. До начала зависимого этапа остаются implementation evidence соответствующего gate и явно перечисленные blocker decisions; они не являются альтернативными архитектурными решениями.
...
- статусы документов;
- ссылки на ADR;
- feature IDs, blocker IDs и stage gates;
