...
sequenceDiagram
    ...
    alt unknown, invalid or inactive
        ...
        UserRepo->>DB: SELECT status/version-counter авторизации FOR UPDATE
...
sequenceDiagram
    ...
    UserRepo->>DB: UPDATE status/version-counter авторизации
...
sequenceDiagram
    ...
    alt invalid or excessive quantity
        ...
        Audit->>DB: INSERT event
        ReturnUC->>Outbox: Insert SaleReturnCompleted
        Outbox->>DB: INSERT event
...
sequenceDiagram
    ...
    Worker->>DB: Mark legacy review-ready transport label or FAILED
    Worker->>DB: COMMIT
...
## Канонические contracts для всех sequence diagrams
