...
### 10.4 Browser transport и storage
...
### 10.5 Rotation, reuse и invalidation
...
- migration role;
- API runtime role и worker runtime role с отдельными credentials/DSN;
- backup role;
...
Runtime также не получает table-level `DELETE`. Worker не имеет SELECT на password или refresh-token hashes; outbox retention разрешён только через две `SECURITY DEFINER` functions с фиксированным terminal status, terminal timestamp и batch limit. `user_sessions` хранит idle/absolute expiry, generation, authentication method и MFA level, а composite FK rotation не допускает cross-user/cross-family predecessor. Migration metadata остаётся read-only. Composite FK `(actor_session_id, actor_user_id)` гарантирует, что audit session принадлежит указанному actor user.
